...
class SearchNoteAdapter(
    val context: Context,
    private val layoutInflater: LayoutInflater,
    private val query: String
) :
    RecyclerView.Adapter<SearchNoteAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            val note = (noteDao.searchNoteByText(query)+noteDao.searchNoteByTitle(query)).toSet().toList()[position]
    ...
    override fun getItemCount()=(noteDao.searchNoteByText(query)+noteDao.searchNoteByTitle(query)).toSet().toList().size
